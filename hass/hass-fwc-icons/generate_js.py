...
SVG = "svg"
OUTFILE = join("dist", "hass-fwc-icons.js")
...
template = Template(
    """const FWC_ICONS_MAP = $icons;

async function getIcon(name) {
  return {path: FWC_ICONS_MAP[name]?.path};
}
async function getIconList() {
  return Object.entries(FWC_ICONS_MAP).map(([icon, content]) => ({
    name: icon,
...
window.customIcons["fwc"] = { getIcon, getIconList };

window.customIconsets = window.customIconsets || {};
window.customIconsets["fwc"] = getIcon;
"""
)
